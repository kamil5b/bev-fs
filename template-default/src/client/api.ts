...
export const userAPI = {
  ...
  async getById(id: number): Promise<UserAPI.GetByIdResponse> {
    const res = await fetch(`${BASE_URL}/users/${id}`);
    return res.json();
  },

  async create(data: UserAPI.CreateRequest): Promise<UserAPI.CreateResponse> {
    const res = await fetch(`${BASE_URL}/users`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
  ...
  async update(id: number, data: UserAPI.UpdateRequest): Promise<UserAPI.UpdateResponse> {
    const res = await fetch(`${BASE_URL}/users/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
  ...
  async delete(id: number): Promise<UserAPI.DeleteResponse> {
    const res = await fetch(`${BASE_URL}/users/${id}`, {
      method: 'DELETE'
    });
    return res.json();
