...
export function loadConfigFromDotEnv() {
  ...
  if (loadedFile) {
    console.debug(`Configuration loaded from: ${loadedFile}`)
  }
...
export function injectConfigToEnv(config: any, prefix = '') {
  for (const key in config) {
    ...
    if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
      ...
      process.env[envKey] = Array.isArray(value)
...
export async function createFrameworkServer(opts: ServerOptions = {}) {
  ...
  // Validate that required configuration is present before startup
  const port = opts.port ?? parseInt(serverConfig.port as string) ?? 3000
  const staticDir =
    opts.staticDir ??
    serverConfig.static_dir ??
    path.join(process.cwd(), 'dist/client')
  const routerDir =
    opts.routerDir ??
    serverConfig.router_dir ??
    path.join(process.cwd(), 'src/server/router')
  ...
  interface RouteModule {
    ...
    middleware?:
      | MiddlewareFunc
      | MiddlewareFunc[]
      | Record<string, MiddlewareFunc | MiddlewareFunc[]>
  }

  /**
   * Separate route-level middleware from method-level middleware
   */
  const separateMiddleware = (
    middleware: any,
  ): {
   ...
   */
  const collectParentMiddleware = async (
    pathParts: string[],
  ): Promise<MiddlewareFunc[]> => {
    ...
    for (let i = 0; i < pathParts.length - 1; i++) {
      ...
      if (resolvedPath) {
        try {
          ...
          if (isDev) {
            console.warn(
              `Failed to load parent middleware from ${resolvedPath}:`,
              e,
            )
          }
        }
  ...
  if (fs.existsSync(routerDir)) {
    try {
      ...
      const registeredRoutes: Set<string> = new Set()
          ...
          if (stat.isDirectory()) {
            ...
            if (resolvedPath) {
              try {
                ...
                const parentMiddlewares =
                  await collectParentMiddleware(pathParts)

                // Separate current route's middleware
                const currentMiddleware = mod.middleware
                  ? separateMiddleware(mod.middleware)
                  : { routeLevel: [], methodLevel: {} }

                // Combine parent + route-level middleware (in order)
                const allRouteMiddlewares = [
                  ...parentMiddlewares,
                  ...currentMiddleware.routeLevel,
                ]
                ...
                if (mod.default) {
                  ...
                  registeredRoutes.add(`DEFAULT ${route}`)
                }

                // Register HTTP method handlers
                const HTTP_METHODS = ['GET', 'POST', 'PUT', 'PATCH', 'DELETE']
                for (const verb of HTTP_METHODS) {
                  const handler = mod[verb as keyof RouteModule] as
                    | RouteHandlerFunc
                    | undefined
                  if (handler) {
                    const methodSpecificMiddleware =
                      currentMiddleware.methodLevel[verb] || []
                    const allMethodMiddlewares = [
                      ...allRouteMiddlewares,
                      ...methodSpecificMiddleware,
                    ]

                    if (allMethodMiddlewares.length > 0) {
                      const methodScopedApp = new Elysia({
                        prefix: route as any,
                      })
                    ...
                    registeredRoutes.add(`${verb} ${route}`)
                    if (isDev) {
                      const mwInfo =
                        allMethodMiddlewares.length > 0
                          ? ` (${allMethodMiddlewares.length} middlewares)`
                          : ''
                ...
                failedRoutes.push({
                  path: resolvedPath || nextPath,
                  error: errorMsg,
                })
                console.warn(
                  `Failed to load route handler at ${resolvedPath}:`,
                  error,
                )
                if (isDev) {
      ...
      // Log summary of route registration (always in development, only if failures in production)
      if (registeredRoutes.size > 0 || failedRoutes.length > 0) {
        console.log(`\n✓ Route Discovery Summary:`)
        console.log(`  Registered: ${registeredRoutes.size} routes`)
        if (failedRoutes.length > 0) {
          // Always warn about failed routes, even in production
          console.warn(`  Failed: ${failedRoutes.length} routes`)
          failedRoutes.forEach(({ path, error }) => {
            console.warn(`    - ${path}: ${error}`)
          })
          // In production, log which routes failed for debugging
          if (!isDev) {
            console.warn(
              '⚠️ Some route handlers failed to load. Check configuration and route files.',
            )
          }
        }
      }
      ...
      if (error instanceof RouteDiscoveryError) {
        ...
        console.error(
          'Failed to auto-register API handlers:',
