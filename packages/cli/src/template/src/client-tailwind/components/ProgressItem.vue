<template>
  <div class="card p-4 hover:shadow-lg transition-shadow">
    <div class="flex justify-between items-start mb-3">
      <span class="badge" :class="`badge-${progress.status}`">{{
        progress.status
      }}</span>
    ...
    <p class="text-sm font-semibold text-slate-900 mb-4">
      Progress: <span class="text-indigo-600">{{ progress.percentage }}%</span>
    </p>
    ...
    <div class="flex gap-2">
      <button @click="$emit('edit')" class="btn btn-sm btn-primary flex-1">
        Edit
      </button>
      <button @click="$emit('delete')" class="btn btn-sm btn-danger flex-1">
        Delete
      </button>
    </div>
  </div>
